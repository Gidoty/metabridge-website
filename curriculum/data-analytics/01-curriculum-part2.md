...
**Lab 11.2: Regression Analysis**
- Tool: Excel or Python (Jupyter Notebook — optional)
- Dataset: Nigerian state education and economic data
- Task: Run a multiple regression with literacy rate as outcome and predictors: education spending per capita, urban population %, poverty rate, female school enrollment. Interpret each coefficient. Calculate R². Identify which variables are statistically significant.
- Deliverable: Regression analysis with written interpretation

**Lab 11.3: Customer Segmentation**
- Tool: Excel (K-means via Solver) or Python
- Dataset: Nigerian e-commerce customer data (recency, frequency, monetary)
- Task: Perform RFM analysis. Score each customer 1–5 on each dimension. Create customer segments (Champions, Loyal, At Risk, Lost, New). Visualise the segments. Describe each segment's characteristics.
- Deliverable: Customer segmentation analysis with segment descriptions and recommended strategies
...
**Requirements:**
- Python Jupyter Notebook (or Excel-based model if Python not covered)
...
5. The constant product formula applies to — **Answer: AMMs in DeFi (wrong context — this is Data Analytics course, see actual Q5 below)**
...
- Module 11: Statistics, regression, correlation (4 questions)
